...
namespace cherryblossomtest.Core
{
	static class Markdown
	{
		...
		public static string MakeSuaveText(string message)
		{
			...
			for (int k = 0; k < message.Length; k++)
			{
				float sin = 0.8f + (float)Math.Sin(CherryWorld.visualTimer + k) * 0.5f;
				float sin2 = 0.8f + (float)Math.Sin(CherryWorld.visualTimer + k + (float)Math.PI / 1.5f) * 0.5f;
				float sin3 = 0.8f + (float)Math.Sin(CherryWorld.visualTimer + k + (float)Math.PI / 1.5f * 2) * 0.5f;
				var color = new Color(sin, sin2, sin3);
				var off = new Vector2(0, 12 + (float)(Math.Sin(CherryWorld.visualTimer * 2 + k / 2f) * 4));

				float sin4 = 0.8f + (float)Math.Sin(CherryWorld.visualTimer + k * 0.25f) * 0.2f;
