...
namespace cherryblossomtest.Helpers
{
    public static partial class Helper
    {
        ...
        public static Color IndicatorColor => Color.White * (float)(0.2f + 0.8f * (1 + Math.Sin(CherryWorld.visualTimer)) / 2f);
        ...
        {
            Player.GetModPlayer<CherryPlayer>().rotation = value;
        }
        ...
        public static bool IsFleshy(NPC NPC)
        {
            return !
                (
                    ...
                    NPC.HitSound == new SoundStyle($"{nameof(cherryblossomtest)}/Sounds/VitricBoss/ceramicimpact")
                );
        }
        ...
        public static SlotId PlayPitched(string path, float volume, float pitch, Vector2? position = null)
        {
            ...
            var style = new SoundStyle($"{nameof(cherryblossomtest)}/Sounds/{path}")
            {
                Volume = volume,
                Pitch = pitch,
                MaxInstances = 0
